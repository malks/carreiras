...
def import_requisitions(jobs,conn):
    for job in jobs:
        ...
        values = "','".join(map(string_and_strip,list(job.values())))
        updates = ",".join(map(mount_updates,list(job.keys()),list(job.values())))
        sql="INSERT IGNORE INTO lunellicarreiras.requisitions ("+keys+") VALUES ('"+values+"') "
        run_sql(sql,conn)
...
if __name__ == "__main__":
    ...
    units_senior=oc_select("SELECT DISTINCT R030FIL.* FROM R030FIL INNER JOIN R126RQU ON R126RQU.CODFIL=R030FIL.CODFIL AND R126RQU.NUMEMP=R030FIL.NUMEMP WHERE R030FIL.NUMEMP IN (1,2,3,4,5,12,20,32,33) AND R126RQU.DATRQU>='"+last_sync[0]['last_sync'].strftime('%Y-%m-%d')+"' AND SITRQU IN ('0','1')",main_oc_conn)
